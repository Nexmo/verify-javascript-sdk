...
describe('Verify Check', () => {
  it('returns an error if the credentials are not set', () => {
    const n = new Nexmo();
    return assert.isRejected(n.verifyCheck(), 'You need to set credentials');
  });

  it('returns an error if the number is not passed as parameter', () => {
    const n = new Nexmo(config);
    return assert.isRejected(n.verifyCheck(), 'You need to pass a number');
  });

  it('returns an error if the pin is not passed as parameter', () => {
    const n = new Nexmo(config);
    return assert.isRejected(n.verifyCheck({
      number: 123456789,
    }), 'You need to pass a pin code');
  });
});
